...
const ANALYST_QUESTIONS = [
  {
    ...
    query:
      "Compare Nvidia's Data Center revenue between FY2023 and FY2024 and outline the growth percentage.",
  },
  {
    label: "Microsoft Intelligent Cloud Growth",
    query:
      "Compare Microsoft's Intelligent Cloud revenue and Azure growth drivers between FY2022 and FY2023.",
  },
  {
    label: "Apple Revenue Segmentation",
    query:
      "Analyze the product segment breakdown (Services, iPhone, Mac, iPad) of Apple's net sales for FY2023.",
  },
  {
    label: "Apple Risk Comparison",
    query:
      "What were the primary legal, regulatory, and supply chain risk factors disclosed by Apple in its 2023 10-K?",
  },
  {
...
export default function Home() {
  ...
  async function ask(customQuery?: string) {
    ...
    try {
      ...
      setAnswer(
        `**Failed to execute RAG pipeline:** ${err instanceof Error ? err.message : "Unknown connection error"}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="min-height-100vh flex flex-col bg-zinc-950 text-zinc-100 selection:bg-indigo-500/30 selection:text-white">
      {/* Header */}
      <header className="sticky top-0 z-50 border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-md px-4 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="text-lg font-bold tracking-tight">Corrective RAG (CRAG)</h1>
              <p className="text-xs text-zinc-500 font-medium">
                Self-correcting AI agent over SEC filings
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Main content */}
      <div className="flex-1 max-w-7xl w-full mx-auto my-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-7 xl:col-span-8 flex flex-col gap-6">
          ...
          {action && (
            <div
              className={`border rounded-xl p-4 flex gap-3 items-center animate-fadeIn ${
                action === "correct"
            ...
            {!answer && !loading && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-zinc-950/20 rounded-2xl">
                ...
                <p className="text-xs text-zinc-600 mt-2 max-w-[280px] leading-relaxed">
                  Select a sample query above or input a custom prompt. The agent will run
                  evaluation metrics and output a cited executive answer here.
                </p>
              </div>
            )}
                ...
                <article className="text-sm leading-relaxed text-zinc-200">
                  <ReactMarkdown
                    components={{
                      h1: ({ node, ...props }) => (
                        <h1
                          className="text-lg font-bold mt-5 mb-2 text-white border-b border-zinc-800 pb-1"
                          {...props}
                        />
                      ),
                      h2: ({ node, ...props }) => (
                        <h2 className="text-md font-bold mt-4 mb-2 text-white" {...props} />
                      ),
                      h3: ({ node, ...props }) => (
                        <h3 className="text-sm font-bold mt-3 mb-1 text-zinc-200" {...props} />
                      ),
                      p: ({ node, ...props }) => (
                        <p className="mb-3 text-zinc-300 leading-relaxed font-normal" {...props} />
                      ),
                      ul: ({ node, ...props }) => (
                        <ul
                          className="list-disc pl-5 mb-4 text-zinc-300 space-y-1.5 font-normal"
                          {...props}
                        />
                      ),
                      ol: ({ node, ...props }) => (
                        <ol
                          className="list-decimal pl-5 mb-4 text-zinc-300 space-y-1.5 font-normal"
                          {...props}
                        />
                      ),
                      ...
                      code: ({ node, ...props }) => (
                        <code
                          className="bg-zinc-800/80 px-1.5 py-0.5 rounded font-mono text-xs text-indigo-300 border border-zinc-700/50"
                          {...props}
                        />
                      ),
                      pre: ({ node, ...props }) => (
                        <pre
                          className="bg-zinc-950 border border-zinc-900 p-4 rounded-xl font-mono text-xs text-zinc-400 overflow-x-auto my-4 shadow-inner leading-relaxed"
                          {...props}
                        />
                      ),
                      strong: ({ node, ...props }) => (
                        <strong
                          className="font-bold text-white bg-indigo-500/5 px-1 py-0.5 rounded border border-indigo-500/10"
                          {...props}
                        />
                      ),
                      blockquote: ({ node, ...props }) => (
                        <blockquote
                          className="border-l-2 border-indigo-500 bg-indigo-950/10 pl-4 py-1.5 my-3 text-zinc-400 italic rounded-r-lg"
                          {...props}
                        />
                      ),
                    }}
